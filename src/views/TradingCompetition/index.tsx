...
const TradingCompetition = () => {
  ...
  useEffect(() => {
    const fetchUserTradingStats = async () => {
      const res = await fetch(`${API_PROFILE}/users/${account}`)
  ...
  }, [account, userTradingInformation, API_PROFILE])

  useEffect(() => {
    const fetchGlobalLeaderboardStats = async () => {
      const res = await fetch(`${API_PROFILE}/leaderboard/global`)
      const data = await res.json()
    ...
    const fetchTeamsLeaderboardStats = async (teamId: number, callBack: (data: any) => void) => {
      try {
        const res = await fetch(`${API_PROFILE}/leaderboard/team/${teamId}`)
        const data = await res.json()
        callBack(data)
      } catch (e) {
        console.error(e)
