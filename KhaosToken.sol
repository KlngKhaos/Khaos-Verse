...
contract KhaosGovernance is BEP20('KhaosGovernance Token', 'KG') {
    ...
    // Safe Khaos transfer function, just in case if rounding error causes pool to not have enough CAKEs.
    function safeKhaosTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 cakeBal = Khaos.balanceOf(address(this));
        if (_amount > cakeBal) {
            Khaos.transfer(_to, cakeBal);
        } else {
